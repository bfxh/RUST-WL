...
///
/// 2026-09-26 从 `1 << 20`（1 048 576 格）抬到 `1 << 21`（2 097 152 格）：千万粒档首测显示
/// 10M 档的域（10.75 m、h = 0.1）正好**越过旧预算** ⇒ 格边被加倍到 0.2 m ⇒ 每格 ≈63 粒
/// ⇒ 邻域核候选量 ×8 ⇒ 实测整 tick **934 ms**（对照 4.25M 档 81³ 未越预算、逐粒 23.4 ns；
/// 10M 越过后逐粒 94 ns ⇒ **4× 跳变**）。抬高后 10M 档格边回到 h（108³ = 1 259 712 格）。
/// 内存代价：`bins` 4 MB → 8 MB（每格 4 B），相对 10M 档 1.15 GB 显存是零头。
/// ⚠️ 这条规则是 **CPU/GPU 共享**，且**改变分箱可能改变浮点累加序 ⇒ 换哈希**：
/// 是否抬、抬多少按 `ADR 0004` 的换代流程判——先用确定性三哈希 + 金样门做零成本判别。
pub const GRID_MAX_BINS: usize = 1 << 21;
