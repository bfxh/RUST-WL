...
impl GaussianSplatField {
    ...
    ///
    /// **近场牛顿投影细化**（2026-09-21）：一阶估计 `(iso − σ)/|∇σ|` 只在面上精确，
    /// 离开面后**系统性偏**——实测平场：面外 0.1/0.2/0.4 m 处 **1.40 / 1.99 / 5.48×**（偏大，
    /// 因为 `|∇σ|` 随远离核而变小），面内 0.10/0.20/0.40 m 处 **0.79 / 0.67 / 0.53×**（偏小）。
    /// 后果：球停在 `f(心) = r` 而不是"真实距离 = r" ⇒ **沉进面下 ≈0.4·r**（实测 r=0.25 沉 0.093、
    /// r=0.35 沉 0.167）；而盒的**角点落在面上**（f ≈ 0，精确）故不受影响。
    /// 细化：沿 `∇σ` 方向迭代走 `f` 直到贴近面，取**净位移**作距离、**符号取首步**。
    /// 只在 `|f₀| ≤ NEAR_BAND` 的带内做（远场/深埋无接触意义、也无需精度）⇒ 成本可控。
    pub fn sdf(&self, p: Vec3) -> f32 {
        let f0 = self.sdf_first_order(p);
        if f0.abs() > NEAR_BAND {
            return f0;
        }
        // **3 次迭代**（按实测选，不按感觉）：每多一次误差约 ÷3（100% → 33% → 10% → 3.5%）。
        // 实测球静置沉入（探针 ② 段，σ=0.5/cut=16 平场、离面间隙）：
        //   不细化 −0.093 / −0.167（r=0.25 / 0.35）｜**2 次 −0.053 / −0.117**｜**3 次 −0.021 / −0.013**
        // 成本（探针 ⑤ 段，200k 近场查询，含截断 4σ）：不细化 1112 ms｜2 次 3049 ms｜3 次 4130 ms。
        // ⇒ 取 3 次：沉入压到皮肤带量级（~1–2 cm），只比 2 次贵 1.35×。
        let mut q = p;
        for _ in 0..3 {
            let (s, g) = self.density_grad(q);
            let gl = g.length();
            if gl < 1e-6 {
                break;
            }
            let fi = (self.iso - s) / gl;
            q += g * (fi / gl); // ∇σ 指向密度增大侧 ⇒ fi>0 时向里走
        }
        let d = (q - p).length();
        if f0 < 0.0 {
            -d
    ...
    /// 一阶隐式距离（无细化；`sdf` 的首步与远场直接用它）。
    fn sdf_first_order(&self, p: Vec3) -> f32 {
