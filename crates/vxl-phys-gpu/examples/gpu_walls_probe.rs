...
fn gather(
    ...
    let (mut pt, mut nm) = ([Vec3::ZERO; 8], [Vec3::ZERO; 8]);
    for (i, p) in ps.iter().enumerate() {
        let np = FluidSystem::wall_planes_in(ids, h, *p, prov, &mut scratch, &mut pt, &mut nm);
        if np == 0 {
            continue;
        }
        out_ids.push(i as u32);
        for k in 0..np {
            planes.push((pt[k], nm[k]));
        }
...
fn main() {
    ...
    let (i2, st, pl) = gather(&ps, &[v], prov, h);
