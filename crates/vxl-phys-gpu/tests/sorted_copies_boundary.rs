//! **混合场景（流体 + 2b 边界粒子）的可复现性**探针——也是格序副本档在耦合路径上的**准入前置条件**。
//!
//! ## 实测结论（2026-09-26）
//!
//! **这条路径今天不是 run-to-run 可复现的**：同一个 `Packet`、**同一个设备**、同一份输入，
//! `snapshot` 之后 `restore` 回初值再跑一遍 ⇒ **流体段的速度仍有 ~30–340 个分量逐位不同**
//! （**次数随机** ⇒ 扰动种子随机），位置与**边界段**全同。已排除的候选：
//! - **格表溢出**：实测最大格占用 **8**（`cap = 512`）⇒ `canon` 不可能放弃规范化；
//! - **未初始化读**：`dens`/`bins`/`out` 每子步全量写、`press` 由 EOS 按**总数**覆盖（`E.n = cfg.n`）；
//! - **跨设备差异**：控制组（纯流体）在同样的"同设备两遍"下**逐位相同** ⇒ 不是回读/设备实例的问题。
//!
//! ⇒ 待办记在 `PLAN-gpu.md` §23.4（**比 §23.3 更要紧**：它影响每一个 2b 场景的可复现性）。
//!
//! ## 因此本文件分两半
//!
//! - 控制组 [`pure_fluid_same_device_rerun_is_bitwise_identical`]：**判红**（纯流体同设备两遍必须逐位相同）
//!   —— 它把上面那条结论钉成"混合场景**特有**"，而不是"回读或计时器坏了"；
//! - 报告 [`mixed_scene_same_device_rerun_report`]：**只打印不判红**（如实报出不可复现的规模，等 §23.4）。
//!
//! 为什么格序档的准入判据要写在这里：`new_sorted` 在混合场景下**今天会静默退回平铺档**（守门条件
//! 是纯流体）⇒ "两档逐位相同"今天退化成"同一条路跑两遍"，而**那条路本身就不可复现** ⇒ 判据写不出来
//! （会随机红）。§23.3 落地前必须先有 §23.4，否则无法判定"开档后枚举序没被改"。
//!
...
fn mixed_scene_same_device_rerun_report() {
    ...
    println!(
        "混合场景（流体 + 2b 边界）同设备两遍：流体段 {df} / 边界段 {db} 个速度分量逐位不同（共 {tot}）"
    );
    println!(
        "⇒ 已知未修（§23.4）：这条路径今天不是 run-to-run 可复现的 ⇒ 不能拿它当格序档的逐位判据。"
    );
}
