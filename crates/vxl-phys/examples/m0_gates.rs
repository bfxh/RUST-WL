...
fn main() {
    ...
    let g_boxes = g.bodies.len();
    for _ in 0..WARMUP_TICKS {
        g.step();
    }
    let gr = run(&mut g, GATE_TICKS);
    let gp = perf(&gr);
    ...
    let gr2 = run(&mut g2, 120);
    let twin_match = gr2.hash_samples.len() == 2
        && gr.hash_samples.len() >= 2
        && gr2.hash_samples[0].1 == gr.hash_samples[0].1
        && gr2.hash_samples[1].1 == gr.hash_samples[1].1;

    let gate_arena_stable = gr.arena_high_water_first == gr.arena_high_water_last
        && gr.arena_overflows == 0
        && gr.arena_capacity > 0;
    ...
    let s_boxes = s.bodies.len();
    for _ in 0..WARMUP_TICKS {
        s.step();
    }
    let sr = run(&mut s, STRESS_TICKS);
    let sp = perf(&sr);
    let (s_tick, s_hash) = *sr.hash_samples.last().expect("哈希采样非空");
    println!("\n[压力场景（仅报告，M1 顶尖化靶场）] {s_boxes} 全动态密堆 × {STRESS_TICKS} tick");
    println!(
    ...
    if gate_pass {
        ...
        eprintln!(
            "❌ M0 门槛 FAIL —— 阻断（p50_ok {p50_ok} / clean {clean} / twin {twin_match} / arena {gate_arena_stable}）"
        );
        std::process::exit(1);
    }
}
