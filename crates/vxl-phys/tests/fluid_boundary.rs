...
fn body_pushes_water_only_with_boundary_coupling() {
    ...
    // 口径（2026-09-22 重标）：自洽体积标定（Akinci `V_b = 1/Σ_bW`）把浮力修正到 ≈1.0×ρVg 后，
    // 体的入水**更柔和**（不再被过强反作用乱抛）⇒ 该增量从 **+1.635 降到 +0.242 m/s**（末态
    // y 1.199→1.151：1200 密度的体下沉更多，物理更对）。判别力不变：对照档仍是 **+0.000**，
    // 判据仍是"非零 vs 零"；绝对下限只防"两侧都是零"的退化（回归闸，不当事后门）。
    // 判据取**差值**（`d_on > d_off + 0.15`）而不是比值：对照档的残波**随档位变**
    // （实测 release +0.000 / debug +0.171 —— 同一场景 f32 路径不同 ⇒ 残波相位不同），
    // 比值判据跨档不成立；差值判据只在问"2b 比对照**多**搅动了多少"。
    // 主判别量其实是下面的末态：对照档的体落到盆底、2b 档被水托住（相位无关）。
    assert!(
        d_on > d_off + 0.15,
        "2b 应把入水柱的水推开（比对照多 ≥0.15 m/s）：Δ = {d_on:+.3} vs {d_off:+.3}"
    );
