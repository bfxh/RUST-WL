//! bvh_impl：从 bvh.rs 按域拆出（纯搬移，语义未改）。
use super::*;
...
impl DynamicBvh {
    ...
    pub(crate) fn free_node(&mut self, i: u32) {
        self.free.push(i);
    }

    #[inline]
    pub(crate) fn fat(&self, a: &Aabb) -> Aabb {
        self.fat_with(a, self.fat_margin)
    }

    #[inline]
    pub(crate) fn fat_with(&self, a: &Aabb, margin: f32) -> Aabb {
    ...
    pub(crate) fn descend_cost(&self, node: u32, leaf_aabb: &Aabb) -> f32 {
    ...
    /// 祖先链 refit + 旋转平衡。
    pub(crate) fn fix_upwards(&mut self, mut node: u32) {
        while node != NULL {
    ...
    pub(crate) fn fix_upwards_grow(&mut self, mut node: u32) {
        while node != NULL {
            let before = node;
            node = self.balance(node);
    ...
    pub(crate) fn rotate_up(&mut self, a: u32, child: u32) {
    ...
    pub(crate) fn build_range(&mut self, items: &mut [Work], leaf_of: &mut [u32]) -> u32 {
