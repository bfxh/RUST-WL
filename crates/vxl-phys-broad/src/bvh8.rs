...
//! **注**：孤立对拍会**低估**竞技场写读代价（宽叶 39.5 候选在隔离测里只贵 8ms，
//! 生产里贵 7ms/tick）——故最终裁决仍以生产接入实测为准。
//!
...
/// 叶容量。**2**：叶结构恰好一个 cache 行的 3/5（40B），且候选膨胀 ≤2×
/// （1 体/叶则内部节点数翻倍、多一层；4 体/叶则候选膨胀 4×）。
pub const LEAF_CAP: usize = 2;
