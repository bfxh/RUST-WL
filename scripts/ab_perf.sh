...
#
# ⚠️ 依赖：仓库根的 `scripts/` 已随检出存在；worktree 用独立 `CARGO_TARGET_DIR`
#   （默认 `C:/vxl-ab-target`，可用 `AB_TARGET_DIR` 覆盖）。
...
export CARGO_TARGET_DIR="${AB_TARGET_DIR:-C:/vxl-ab-target}"
...
echo "   rounds=${rounds}｜抓取模式=${pattern}｜CARGO_TARGET_DIR=${CARGO_TARGET_DIR}"
...
extract() { # extract <文件>
    grep -m1 -E "$pattern" "$1" | grep -oE '[0-9]+\.?[0-9]*' | tail -n1
}

# 每臂先**各编一次**（预热 + 保证比的是编译产物，不是编译时间）。
( cd "$wt" && "$@" >/dev/null 2>&1 )
( cd "$root" && "$@" >/dev/null 2>&1 )

aset=""; bset=""
for i in $(seq 1 "$rounds"); do
    ( cd "$wt"   && "$@" ) >/tmp/ab_a.log 2>&1
    a="$(extract /tmp/ab_a.log)"
    ( cd "$root" && "$@" ) >/tmp/ab_b.log 2>&1
