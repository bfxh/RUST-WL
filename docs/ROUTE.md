...
| **液体** | **双向 ✅（2b 落地 2026-09-22：Akinci 两层边界粒子；示例 `fluid_buoyancy 2b` + 测试 `fluid_boundary`）——适用边界：体 ≳2h 才准（默认档 h=0.1 ⇒ ≳0.2 m），记账见 `M1-EXIT.md` §4** | 地形/侵蚀（后续） | 双向（同通道） | 自碰撞 | 核同构（§3.1-3） | 风驱动表面 |
| **喷溅** | 场/代理 | 场 | 代理 | 核同构 | 聚类 | — |
| **风** | 气动 | — | 主耦合 | 表面驱动 | — | — |
