...
**破坏路径的已知限制（2026-09-14 实测，如实；三个真问题两个已定位到函数）**：
① **碎块逃逸（未修）**：挖出的碎块会「自由落体穿地」——根因候选 = 体素 SDF 查询是
**局域 ±1 格 + 0.75 m 截断**，大碎块（实测最大 56 t、数米见方）的采样点落在查询
范围外 ⇒ 拿不到接触 ⇒ 直接落下（`m3_impact` 的「逃逸」计数；修法：大对象改走
「盒 AABB × 体素格」的专用接触，而不是点采样）。② **CCD × 静止接触互锁（未修）**：
贴地滑行的体每一扫描采样都判「命中」⇒ 被钳回起点、原地锁死（`ccd_pass` 的命中
判据需细化为「只对新出现的/更深的接触钳位」）。③ **`fill_box` 闭区间陷阱（已修）**：
max 侧改开区间（`max=1.5` 现在真的填到顶面 1.5，而非多填一格 [1.5,2.0)）——修前
场景会「地板比预期厚一格」（炮弹开局嵌在地板里、tick 1 就触发破坏，实测踩中）。
另：仅支持轴对齐盒域挖掘（任意形状切割 / Voronoi 预断裂待做）。
