...
**破坏路径的已知限制（2026-09-14 实测，如实）**：① 挖出的碎块若与残余结构重叠，
会被位置修正挤出、可能隧穿逃逸（`m3_impact` 汇总里的「逃逸」数；修法候选：只挖
完全落在挖域内的格 / 碎块微小外偏 / 小碎块走「无碎块销毁」）；② 高速冲击体需
CCD（`ccd_speed_threshold` 默认 ∞；20 m/s 弹体开 CCD 后逐采样最深 0.000 ✓，
12 m/s 恰好落在判据边界外 ⇒ 会隧穿皮肤带）；③ 目前只支持轴对齐盒域挖掘
（任意形状切割 / Voronoi 预断裂待做）。
