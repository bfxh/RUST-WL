...
**复核读数（2026-09-20 会话末，形状家族改动之后）**——本会话改的都是窄相，金样是"真实度"那条轴
的门，故全会话末复核一次（600 tick 末态）：

| 场景 | vxl 末态 | Rapier 同列 | 判定 |
|---|---|---|---|
| col45 | **45/45 入睡**、KE 0.0、最深 0.000、y 带 [0.25, 2.25] | 45/45、最深 −0.000、y 2.24 | ✅ 通过 |
| pile5 | **1735/2000 入睡**、KE 9.9、最深 0.006、y 带完整 | 2000/2000、最深 −0.000 | ✅ 与既录一致 |
| tower25 | **0/2500 入睡**、\|v\|max 0.224、最深 0.013、y 带 [0.25, 12.13]（塔身完整） | 2500/2500、最深 0.025 | ⚠️ 已知 P1 睡眠缺口，**非回退** |

⇒ 本会话的窄相改动（胶囊解析路 / 圆锥 / 复合体 / 棱柱支撑 / 两种高度场路径）**未触动保真门**；
入睡列的缺口仍是 `OPEN-PROBLEMS.md` P1 那条（tower25 0/2500、pile5 未满），与形状工作无关。
...
⇒ **判睡眠一律看"超阈体数"**。
升为默认配方前须过：四哈希 + 金样三场景 + 稳定性判据 + arena 自检 19/0/0。
