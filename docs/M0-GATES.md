...
- 相位 arena：**水位 8192B 首末一致、分配 10 次、溢出 0**（§0.1 #10 平稳性✅）
  ⚠️ **容量读数不是稳定量**（2026-09-22 更正，别再当回归追）：`PhaseArena::capacity()`
  = `HASH_SCRATCH_BYTES(8192) + MAX_ALIGN − base`，其中 `base` 是该缓冲**在堆上的地址未对齐偏移**
  （分配器地址的函数）⇒ 实测在 8208–8224 之间跳：同日只加了一个配置字段、改了堆布局，
  读数就从 8224 变 8208，**代码语义没变**。门判的是 `stable`（水位首末一致 + 溢出 0）✅，
  **不要钉容量绝对值**。
