...
**下一步（重排后的优先级）**：① **T1 绳索最小闭环**（XPBD 距离约束 + 点-形状接触，不碰三角网自碰撞）
—— 布料那条线的真正起点，现在"胶囊/圆柱/圆锥 vs 任何地形"都已通，绳索段可以直接用胶囊挂在网上；
② `provider vs provider` 与**三角网自碰撞**（仍是空白，属于"三角形一等几何"那条大线）；
③ T3-③ 预判式碎裂调度、T2 转换物理四件套（见 §1–§3 的排序）。
